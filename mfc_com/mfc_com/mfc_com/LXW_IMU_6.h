...
{
	float rot_xx= float(Pitch_angle0)*3.1415926/180;//弧度 单位
	float rot_yy= float(Rool_angle0)*3.1415926/180;
...
/******************************************/
void Quarter_T_OlA(float w,float x,float y,float z)
{
	////方向余弦矩阵寄存器
	//float t11;
	//float t12;
	...
	Rool=atan2(2*(w*x+y*z),1-2*(x*x+y*y));
	Pitch=asin(2*(w*y-x*z));
	Yaw=atan2(2*(x*y+w*z),1-2*(z*z+y*y));
