...
void CCommTest::OnCommMscomm1()
{
	...
	if(m_mscomm.get_CommEvent()==2)//值为 2 表示接收缓冲区内有字符
	{
		...
		if(command_a=='X' && command_b=='W')//帧头校验
		{
			...
			{
				FILE *file=fopen(".//NiHe//Compass_Data_Blue.txt","a+");
				{
...
void CCommTest::OnBnClickedButtonOpen()
{
	...
	m_mscomm.put_CommPort(2);//选择COM1
	m_mscomm.put_InBufferSize(1024);//接收缓冲区
	m_mscomm.put_OutBufferSize(1024);//发送缓冲区 
	m_mscomm.put_InputLen(0);//设置当前接收区数据长度为0,表示全部读取
	m_mscomm.put_InputMode(1);//以二进制方式读写数据
	m_mscomm.put_RThreshold(26+24);//26+36//接收缓冲区有1个及1个以上字符时，将引发接收数据的OnComm事件
...
void CCommTest::OnTimer(UINT_PTR nIDEvent)
{
	...
	//磁场航向
	Yaw_angle_S= GLB_Angle_Compass_YawB(float(Compass_X),float(Compass_Y),float(Compass_Z),260,Pitch_angle,Rool_angle,Yaw_angle);

	Yaw_angle=Yaw_angle_S;

	Yaw_angle_Offset0=Yaw_angle;//获取航向角矫正偏移量
	Yaw_angle-=Yaw_angle_Offset;//航向角矫正

	OlA_T_Quarter(Rool_angle,Pitch_angle,Yaw_angle);

#if 0

	for(int i=5;i>0;i--)
	{
		q0_data[i]=q0_data[i-1];
		q1_data[i]=q1_data[i-1];
		q2_data[i]=q2_data[i-1];
		q3_data[i]=q3_data[i-1];
	}
	...
	float t=0.533;
		...
		{
			double anglef;
			double dot_data=q0_data[i]*q0+q1_data[i]*q1 + q2_data[i]*q2 +q3_data[i]*q3;
			
			anglef=acos(dot_data);
				
			//anglef=abs(anglef);
			if((anglef)!=0)
			{
				float t1=(t*anglef)/(anglef+0.000001);
				float t2=((1-t)*anglef)/(anglef+0.000001);
				float q0r=q0_data[i]*t1+q0*t2;
				float q1r=q1_data[i]*t1+q1*t2;
				float q2r=q2_data[i]*t1+q2*t2;
				float q3r=q3_data[i]*t1+q3*t2;

				float norm = sqrt(q0r*q0r + q1r*q1r + q2r*q2r + q3r*q3r);
				q0 = q0r / norm;
				q1 = q1r / norm;
				q2 = q2r / norm;
				q3 = q3r / norm;
   ...
   slerp( result, starting, ending, 0.5999 );
   ...
   q3b=result[2];
	...
	if(abs(Rotx)>0.0365 | abs(Roty)>0.0365 | abs(Rotz)>0.0265)
	{
		Q0=q0; Q1=q1;Q2=q2;Q3=q3; //传感器四元数坐标系 转 OpenGL坐标系
	}
		...
		m_openGL.RenderGLScene(Q0,Q1,Q2,Q3,G0_Pitch,G0_Roll,Hx,Hy,Rool_angle_S,Pitch_angle_S, Yaw_angle_S,Rool_angle,Pitch_angle, Yaw_angle,
			                   1,1,w,Q1,Q2,Q3,Mccx,1,pos_x_Glob,pos_y_Glob,pos_z_Glob,Yaw_angle_Offset0,Yaw_angle_Offset,
